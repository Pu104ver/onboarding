...
const NewEmployeeForm = ({setToken, openTokenModal, closeModal, role}: INewEmployeeFormProps) => {
  ...
  const onSubmit: SubmitHandler<INewEmployeeForm> = async fields => {
    ...
    await createEmployee({
      ...
      if (data) {
        setToken(`${data.uid}_${data.access_token}`);
        openTokenModal();
        ...
        <Input
          ...
          {...register('full_name', {
            ...
            validate: value =>
              ...
              checkOnlyLetters(value, `ФИО ${roleText} не может содержать спецсимволы и цифры`),
          })}
        />
        {errors.full_name && (
        ...
        <Controller
          ...
          render={({field: {value, onChange}}) => (
            <>
              ...
              {errors.projects && <p className={styles.errorMsg}>Выберите проект</p>}
            </>
          )}
        />
      </label>
