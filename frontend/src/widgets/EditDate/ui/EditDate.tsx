...
import {DATE_FOR_BACKEND, RUS_DATE, RUS_DATE_WITH_TIME} from '@/shared/const/DateStrings';
...
const EditDate = ({field, changeData, initialDate, showTimeSelect, ...props}: EditProps) => {
  ...
  const handleChange = () => {
    date && changeData(field, showTimeSelect ? date.toISOString() : format(date, DATE_FOR_BACKEND));
    setIsChange(false);
  };
