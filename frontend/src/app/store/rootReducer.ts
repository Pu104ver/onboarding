...
import {logout, userReducer} from '@/entities/User/model/slice/UserSlice';

const jwtTokenRefresher: Middleware =
  ({dispatch}) =>
  next =>
  action => {
    ...
    if (isRejectedWithValue(action)) {
      if (typedAction.payload.status === 401) {
        ...
        if (refresh_token) {
          baseApi.endpoints.getNewAccessToken.initiate({refresh_token});
        } else {
          handleLogout();
        }
      }
...
export const store = configureStore({
  ...
  middleware: getDefaultMiddleware =>
    getDefaultMiddleware()
      .concat(jwtTokenRefresher, baseApi.middleware)
