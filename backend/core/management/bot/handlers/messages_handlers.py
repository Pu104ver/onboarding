...
from core.management.bot.texts import texts_reg_success_code, texts_start_is_registered, texts_reg_invalid_code, \
    texts_start_not_registered, texts_reg_success_code_curator, texts_for_curator
    ...
    if not employee:
        ...
        if answer:
            if employee.role == Employee.RoleChoices.EMPLOYEE:
                ...
                await bot.send_document(message.from_user.id, caption=texts_reg_success_code.format(curator_full_name), document=book_employee, parse_mode='HTML', reply_markup=ReplyKeyboardRemove())
            else:
                await bot.send_message(message.from_user.id, text=texts_reg_success_code_curator, parse_mode='HTML', reply_markup=ReplyKeyboardRemove())
            await state.clear()
