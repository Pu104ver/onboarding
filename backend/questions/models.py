...
class CategoryAnalytics(models.TextChoices):
    ...
    ANSWER_SPEED = "answer_speed", "Скорость ответа"
    PRESENCE_MEETING = "presence_meeting", "Присутствие на встречах"
...
class Question(models.Model):
    ...
    category_analytics = models.CharField(
        max_length=30,
        choices=CategoryAnalytics.choices,
        default=None,
        blank=True,
        null=True,
...
class QuestionCondition(models.Model):
    ...
    question = models.ForeignKey(
        Question, related_name="conditions", on_delete=models.CASCADE, verbose_name="Вопрос"
    )

    previous_question = models.ForeignKey(
        Question, related_name="previous_conditions", on_delete=models.CASCADE, verbose_name="Предыдущий вопрос"
    )
    answer_condition = models.CharField(max_length=255, choices=AnswerType.choices, verbose_name="Условие")
...
class KeyboardType(models.Model):
    ...
    question_type = models.CharField(
        max_length=50, choices=QuestionType.choices, unique=True, verbose_name="Тип вопроса"
    )
